...
func TestVxlanInnerIpv4(t *testing.T) {
	...
	api := otg.NewOtgApi(t)
	c := vxlanInnerIpv4Config(api, testConst)
	...
	api.WaitFor(
		func() bool { return vxlanInnerIpv4FlowMetricsOk(api, testConst) },
		&otg.WaitForOpts{FnName: "WaitForFlowMetrics"},
	)

	api.StopCapture()

	vxlanInnerIpv4CaptureOk(api, c, testConst)
}

func vxlanInnerIpv4Config(api *otg.OtgApi, tc map[string]interface{}) gosnappi.Config {
	c := api.Api().NewConfig()
...
func vxlanInnerIpv4CaptureOk(api *otg.OtgApi, c gosnappi.Config, tc map[string]interface{}) {
	if !api.TestConfig().OtgCaptureCheck {
		return
	}
	ignoredCount := 0
