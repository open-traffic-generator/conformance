...
func TestUdpPortValues(t *testing.T) {
	...
	api := otg.NewOtgApi(t)
	c := udpPortValuesConfig(api, testConst)
	...
	api.WaitFor(
		func() bool { return udpPortValuesFlowMetricsOk(api, testConst) },
		&otg.WaitForOpts{FnName: "WaitForFlowMetrics"},
	)

	api.StopCapture()

	udpPortValuesCaptureOk(api, c, testConst)
}

func udpPortValuesConfig(api *otg.OtgApi, tc map[string]interface{}) gosnappi.Config {
	c := api.Api().NewConfig()
...
func udpPortValuesFlowMetricsOk(api *otg.OtgApi, tc map[string]interface{}) bool {
...
func udpPortValuesCaptureOk(api *otg.OtgApi, c gosnappi.Config, tc map[string]interface{}) {
	if !api.TestConfig().OtgCaptureCheck {
		return
	}
	ignoredCount := 0
	...
	for i := 0; i < len(cPackets.Packets); i++ {
		...
		// udp header
		j := i - ignoredCount
		cPackets.ValidateField(t, "udp src", i, 34, api.Uint64ToBytes(uint64(txUdpPortValues[j%len(txUdpPortValues)]), 2))
		cPackets.ValidateField(t, "udp dst", i, 36, api.Uint64ToBytes(uint64(rxUdpPortValues[j%len(rxUdpPortValues)]), 2))
