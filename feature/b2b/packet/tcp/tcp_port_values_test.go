...
func TestTcpPortValues(t *testing.T) {
	testConst := map[string]interface{}{
		...
		"txIp":            "1.1.1.1",
		"rxIp":            "1.1.1.2",
		"txTcpPortValues": []uint32{5000, 5010, 5020, 5030},
		"rxTcpPortValues": []uint32{6000, 6010, 6020, 6030},
	}

	api := otg.NewOtgApi(t)
	c := tcpPortValuesConfig(api, testConst)
	...
	api.WaitFor(
		func() bool { return tcpPortValuesFlowMetricsOk(api, testConst) },
		&otg.WaitForOpts{FnName: "WaitForFlowMetrics"},
	)

	api.StopCapture()

	tcpPortValuesCaptureOk(api, c, testConst)
}

func tcpPortValuesConfig(api *otg.OtgApi, tc map[string]interface{}) gosnappi.Config {
	c := api.Api().NewConfig()
...
func tcpPortValuesCaptureOk(api *otg.OtgApi, c gosnappi.Config, tc map[string]interface{}) {
	if !api.TestConfig().OtgCaptureCheck {
		return
	}
	ignoredCount := 0
	txTcpPortValues := tc["txTcpPortValues"].([]uint32)
	rxTcpPortValues := tc["rxTcpPortValues"].([]uint32)
