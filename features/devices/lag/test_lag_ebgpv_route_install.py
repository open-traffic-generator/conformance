...
def test_lag_ebgpv4_route_install():
    ...
    api = otg.OtgApi()
    c = ebgpv4_route_install_config(api, test_const)
    ...
    api.wait_for(
        fn=lambda: lag_metrics_ok(api, test_const),
        fn_name="wait_for_lag_metrics",
    )
...
def ebgpv4_route_install_config(api, tc):
    c = api.api.config()
    ptx = c.ports.add(name="ptx", location=api.test_config.otg_ports[0])
    ptx2 = c.ports.add(name="ptx2", location=api.test_config.otg_ports[2])
    prx = c.ports.add(name="prx", location=api.test_config.otg_ports[1])
    prx2 = c.ports.add(name="prx2", location=api.test_config.otg_ports[3])

    ly = c.layer1.add(name="ly", port_names=[ptx.name, prx.name, ptx2.name, prx2.name])
    ly.speed = api.test_config.otg_speed

    lag1, lag2 = c.lags.lag(name="lag1").lag(name="lag2")

    lag1.protocol.lacp.actor_system_id = "01:01:01:01:01:01"
    lag1.protocol.lacp.actor_system_priority = 1
    lag1.protocol.lacp.actor_key = 1

    lag2.protocol.lacp.actor_system_id = "01:01:01:01:01:02"
    lag2.protocol.lacp.actor_system_priority = 1
    lag2.protocol.lacp.actor_key = 1

    l1p1, l1p2 = lag1.ports.port(port_name=ptx.name).port(port_name=ptx2.name)
    l2p1, l2p2 = lag2.ports.port(port_name=prx.name).port(port_name=prx2.name)

    l1p1.ethernet.name = "p1_lag1_eth"
    l1p1.ethernet.mac = "00:00:00:00:00:16"
    l1p1.lacp.actor_port_number = 1
    l1p1.lacp.actor_port_priority = 1
    l1p1.lacp.actor_activity = "active"

    l1p2.ethernet.name = "p2_lag1_eth"
    l1p2.ethernet.mac = "00:00:00:00:00:18"
    l1p2.lacp.actor_port_number = 2
    l1p2.lacp.actor_port_priority = 1
    l1p2.lacp.actor_activity = "active"

    l2p1.ethernet.name = "p1_lag2_eth"
    l2p1.ethernet.mac = "00:00:00:00:00:17"
    l2p1.lacp.actor_port_number = 1
    l2p1.lacp.actor_port_priority = 1
    l2p1.lacp.actor_activity = "active"

    l2p2.ethernet.name = "p2_lag2_eth"
    l2p2.ethernet.mac = "00:00:00:00:00:19"
    l2p2.lacp.actor_port_number = 1
    l2p2.lacp.actor_port_priority = 1
    l2p2.lacp.actor_activity = "active"

    dtx = c.devices.add(name="dtx")
    drx = c.devices.add(name="drx")

    dtx_eth = dtx.ethernets.add(name="dtx_eth")
    dtx_eth.port_name = lag1.name
...
def lag_metrics_ok(api, tc):
    for m in api.get_lag_metrics():
        if m.oper_status == m.DOWN:
            return False
    return True
