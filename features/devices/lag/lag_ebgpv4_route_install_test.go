...
func TestLagEbgpv4RouteInstall(t *testing.T) {
	...
	api.WaitFor(
		func() bool { return LagMetricsOk(api, testConst) },
		&otg.WaitForOpts{FnName: "WaitForLagMetrics"},
	)
...
func ebgpv4RouteInstallConfig(api *otg.OtgApi, tc map[string]interface{}) gosnappi.Config {
	...
	lag1 := c.Lags().Add().SetName("lag1")
	lag2 := c.Lags().Add().SetName("lag2")
	...
	lp1 := lag1.Ports().Add().SetPortName(ptx.Name())
	lp2 := lag2.Ports().Add().SetPortName(prx.Name())

	lp1.Ethernet().SetName("p1_lag_eth").SetMac("00:00:00:00:00:16")
	lp1.Lacp().SetActorPortNumber(1).SetActorPortPriority(1).SetActorActivity("active")

	lp2.Ethernet().SetName("p2_lag_eth").SetMac("00:00:00:00:00:17")
	lp2.Lacp().SetActorPortNumber(1).SetActorPortPriority(1).SetActorActivity("active")
...
func LagMetricsOk(api *otg.OtgApi, tc map[string]interface{}) bool {
	for _, m := range api.GetLagMetrics() {
		if m.OperStatus() == gosnappi.LagMetricOperStatus.DOWN {
			return false
		}
	}
	return true
