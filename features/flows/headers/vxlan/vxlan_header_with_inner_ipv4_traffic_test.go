...
func TestVxlanHeaderIPv4(t *testing.T) {
	...
	api := otg.NewOtgApi(t)
	c := vxLanHeaderConfigIPv4(api, testConst)
	...
	api.WaitFor(
		func() bool { return vxLanHeaderMetricsOkIPv4(api, testConst) },
		&otg.WaitForOpts{FnName: "WaitForFlowMetrics"},
	)

	api.StopCapture()

	vxLanHeaderCaptureOkIPv4(api, c, testConst)
}

func vxLanHeaderConfigIPv4(api *otg.OtgApi, tc map[string]interface{}) gosnappi.Config {
	c := api.Api().NewConfig()
...
func vxLanHeaderMetricsOkIPv4(api *otg.OtgApi, tc map[string]interface{}) bool {
...
func vxLanHeaderCaptureOkIPv4(api *otg.OtgApi, c gosnappi.Config, tc map[string]interface{}) {
	if !api.TestConfig().OtgCaptureCheck {
		return
	}
	ignoredCount := 0
