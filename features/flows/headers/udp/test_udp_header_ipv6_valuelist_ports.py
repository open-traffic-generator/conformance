...
def test_udp_header_valuelist_ports():
    test_const = {
        "pktRate": 50,
        "pktCount": 100,
        "pktSize": 128,
        ...
        "txIp": "2000::1",
        "rxIp": "2000::2",
        "txUdpPortValueList": [5000, 5010, 5020, 5030],
        "rxUdpPortValueList": [6000, 6010, 6020, 6030],
    }

    api = otg.OtgApi()
    c = udp_header_ipv6_valuelist_ports_config(api, test_const)
...
def udp_header_ipv6_valuelist_ports_config(api, tc):
    c = api.api.config()
    ...
    eth, ip, udp = f1.packet.ethernet().ipv6().udp()
    ...
    udp.src_port.values = tc["txUdpPortValueList"]
    udp.dst_port.values = tc["rxUdpPortValueList"]
...
def capture_ok(api, c, tc):
    ...
    for i, p in enumerate(captured_packets.packets):
        ...
        # ipv6 header
        captured_packets.validate_field(
            "ipv6 src", i, 22, api.ipv6_addr_to_bytes(tc["txIp"])
        )
        captured_packets.validate_field(
            "ipv6 dst", i, 38, api.ipv6_addr_to_bytes(tc["rxIp"])
        )
        captured_packets.validate_field(
        ...
        captured_packets.validate_field(
            ...
            54,
            api.num_to_bytes(
                tc["txUdpPortValueList"][reqPacketIdx % len(tc["txUdpPortValueList"])],
                2,
            ),
        )
        captured_packets.validate_field(
            ...
            56,
            api.num_to_bytes(
                tc["rxUdpPortValueList"][reqPacketIdx % len(tc["rxUdpPortValueList"])],
                2,
            ),
        )
        captured_packets.validate_field(
            "udp length", i, 58, api.num_to_bytes(tc["pktSize"] - 14 - 4 - 40, 2)
        )
