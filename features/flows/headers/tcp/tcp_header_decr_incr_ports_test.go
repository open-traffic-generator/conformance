...
func TestTcpHeaderDecrIncrPorts(t *testing.T) {
	...
	api := otg.NewOtgApi(t)
	c := tcpHeaderDecrIncrPortsConfig(api, testConst)
	api.SetConfig(c)
	api.StartCapture()
	api.StartTransmit()
	api.WaitFor(
		func() bool { return tcpHeaderDecrIncrPortsMetricsOk(api, testConst) },
		&otg.WaitForOpts{FnName: "WaitForFlowMetrics"},
	)
	api.StopCapture()
	tcpHeaderDecrIncrPortsCaptureOk(api, c, testConst)
}

func tcpHeaderDecrIncrPortsConfig(api *otg.OtgApi, tc map[string]interface{}) gosnappi.Config {
	c := api.Api().NewConfig()
	...
	tcp.SrcPort().Decrement().SetStart(tc["txTcpPortStart"].(int32)).SetStep(tc["txTcpPortStep"].(int32)).SetCount(tc["txTcpPortCount"].(int32))
	tcp.DstPort().Increment().SetStart(tc["rxTcpPortStart"].(int32)).SetStep(tc["rxTcpPortStep"].(int32)).SetCount(tc["rxTcpPortCount"].(int32))
	api.Testing().Logf("Config:\n%v\n", c)
	return c
}

func tcpHeaderDecrIncrPortsMetricsOk(api *otg.OtgApi, tc map[string]interface{}) bool {
...
func tcpHeaderDecrIncrPortsCaptureOk(api *otg.OtgApi, c gosnappi.Config, tc map[string]interface{}) {
	if !api.TestConfig().OtgCaptureCheck {
		return
	}
	ignoredCount := 0
