...
func tcpHeaderValuelistPortsCaptureOk(api *otg.OtgApi, c gosnappi.Config, tc map[string]interface{}) {
	...
	for i := 0; i < len(cPackets.Packets); i++ {
		...
		// tcp header
		cPackets.ValidateField(t, "tcp src", i, 34, api.Uint64ToBytes(uint64(tc["txTcpPortValueList"].([]int32)[reqPacketIdx%len(tc["txTcpPortValueList"].([]int32))]), 2))
		cPackets.ValidateField(t, "tcp dst", i, 36, api.Uint64ToBytes(uint64(tc["rxTcpPortValueList"].([]int32)[reqPacketIdx%len(tc["rxTcpPortValueList"].([]int32))]), 2))
	}
