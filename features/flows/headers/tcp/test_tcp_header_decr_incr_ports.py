...
def tcp_header_dec_src_inc_dst_ports_config(api, tc):
    ...
    tcp.dst_port.increment.start = tc["rxTcpPortStart"]
    tcp.dst_port.increment.step = tc["rxTcpPortStep"]
    tcp.dst_port.increment.count = tc["rxTcpPortCount"]
    log.info("Config:\n%s", c)
    return c
...
def capture_ok(api, c, tc):
    ...
    for i, p in enumerate(captured_packets.packets):
        ...
        captured_packets.validate_field(
            ...
            34,
            api.num_to_bytes(
                tc["txTcpPortStart"] - (i % tc["txTcpPortCount"]) * tc["txTcpPortStep"],
                2,
            ),
        )
        captured_packets.validate_field(
            ...
            36,
            api.num_to_bytes(
                tc["rxTcpPortStart"] + (i % tc["rxTcpPortCount"]) * tc["rxTcpPortStep"],
                2,
            ),
        )
