...
func TestUdpHeaderDstSrcPortList(t *testing.T) {
	testConst := map[string]interface{}{
		...
		"txIp":          "2000::1",
		"rxIp":          "2000::2",
		"txUdpPortList": []int32{5000, 5001},
		"rxUdpPortList": []int32{6000, 6001},
	}

	api := otg.NewOtgApi(t)
	c := udpHeaderConfigWithList(api, testConst)
	...
	api.WaitFor(
		func() bool { return udpHeaderMetricsWithListOk(api) },
		&otg.WaitForOpts{FnName: "WaitForFlowMetrics"},
	)
}

func udpHeaderConfigWithList(api *otg.OtgApi, tc map[string]interface{}) gosnappi.Config {
	c := api.Api().NewConfig()
	...
	udp.SrcPort().SetValues(tc["txUdpPortList"].([]int32))
	udp.DstPort().SetValues(tc["rxUdpPortList"].([]int32))

	api.Testing().Logf("Config:\n%v\n", c)
	return c
}

func udpHeaderMetricsWithListOk(api *otg.OtgApi) bool {
