...
func TestIpv4Header(t *testing.T) {
	...
	api := otg.NewOtgApi(t)
	c := ipv4HeaderConfig(api, testConst)
	...
	api.WaitFor(
		func() bool { return ipv4HeaderMetricsOk(api) },
		&otg.WaitForOpts{FnName: "WaitForFlowMetrics"},
	)
}

func ipv4HeaderConfig(api *otg.OtgApi, tc map[string]interface{}) gosnappi.Config {
	c := api.Api().NewConfig()
...
func ipv4HeaderMetricsOk(api *otg.OtgApi) bool {
